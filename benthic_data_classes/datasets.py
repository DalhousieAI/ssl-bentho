    ...
    def __getitem__(self, idx):
        ...
        if os.path.isfile(node_file_path):
            ...
            with tempfile.TemporaryDirectory() as dir_tmp:
                ...
                node_file_temp_path = os.path.join(
                    dir_tmp, os.path.basename(row["path"])
                )
...
class BenthicNetDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, tar_dir, annotations=None, transform=None):
        """
        Dataset for BenthicNet data.
...
def get_dataset_by_station_split(
    file, validation_size=0.25, test_size=0.2, replace=False
):
    ...
    fn = lambda obj: obj.loc[
        np.random.choice(obj.index, round(len(obj.index) * test_size), replace), :
    ]
    test_same_idxs = (
        training_data.groupby("label", as_index=False).apply(fn)["index"].tolist()
    )
    ...
    fn = lambda obj: obj.loc[
        np.random.choice(obj.index, round(len(obj.index) * validation_size), replace), :
    ]
    validation_idxs = (
        training_data.groupby("label", as_index=False).apply(fn)["index"].tolist()
    )
